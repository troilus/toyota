...
Service.BRM.prototype.changeToolIcons = function() {
  ...
  try {
    ...
    // 簡易検索が有効で、修理書が存在し、検索キーワード（タイトル名称）、品名コードの
    // いずれかが存在する場合に、修理書へのリンクアイコンを表示する
    if(this.myGlobalInfo.VALID_SIMPLE == "yes"
        && this.myGlobalInfo.SCH_OPT_DEF.indexOf(DictConst.C_SEARCH_OPTION_CD_REPAIR) != -1
