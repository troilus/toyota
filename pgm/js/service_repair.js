      ...
      // サブパラIDが存在しない場合はコンテンツ表示エリアへ表示
        ...
        if(nowHistory.KEY != Service.Repair.TOPPAGE_NAME) {
          ...
          if(Util.$getIndexOfArray(diagCategory,
            ...
            // 診断フローはスクロールバーを出さない
            scr = "no";
            // 履歴用のURLは診断フローのパラグラフ（意見要望で表示する内容）
            url = Service.Repair.CONTENT_PATH.replace("{0}", parId);
            nowHistory.URL = url;
            // 表示用のURLは診断フロー
  ...
  try {
    ...
    if(Util.$getIndexOfArray(diagCategory, cat) != -1
      ...
      Use.Util.$observe($("repair_attentions"), "click",
        (function(own) {
          return function(evt) {
            Service.$doClickCautionNoticeIcn(
                own.getSelectInfo().PAR_ID);
          }
        })(this)
      );
...
Service.Repair.prototype.setHistory = function(paraInfo, isExpand, isFlow) {
  ...
  try {
    ...
    if(paraInfo.CAT == "C") {
      this.selectInfo.PAR = paraInfo.PAR;
    }
 ...
 *                             false: そのほか
 */
Service.Repair.prototype.openProceduresWindow = function(
  ...
  try {
    ...
    // 診断フローからの引継ぎ情報を修理書タブに保持
    if(param != null) {
      Util.$propcopy(param, this.mySuccessionMode3);
    }
    
    Use.Repair.$openUrlProcedures(paraId, anchor, target, 
        Service.Repair.WINDOW_OPTION, Util.WINDOW_SIZE_2, isCallFlow);
