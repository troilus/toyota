...
Service.SearchResult.prototype.setKeywordTxt = function(keywordTxt) {
  ...
  try {
    ...
    if(this.searchType == "1") {
      keywordMaxLen = Use.Util.$getMessage("CONST_RESULT_KEYWORD_LEN");
      keywordTemp = Use.Util.$getMessage("CONST_RESULT_KEYWORD");
      
    // 横串検索の場合の表示文言
    } else if(this.searchType == "3") {
      keywordMaxLen = Use.Util.$getMessage("CONST_RESULT_KEYWORD_LEN_YOKOGUSHI");
      keywordTemp = Use.Util.$getMessage("CONST_RESULT_KEYWORD_YOKOGUSHI");
    }
    
    limitLength = parseInt(keywordMaxLen, 10);
...
Service.SearchResult.prototype.doResetManuals = function() {
  ...
  try {
    ...
    var targets = [DictConst.C_PUB_TYPE_REPAIR, 
                   DictConst.C_PUB_TYPE_NCF, 
                   DictConst.C_PUB_TYPE_EWD, 
                   DictConst.C_PUB_TYPE_BRM];
    var len = targets.length;
    ...
    var rootSuf = "_result";
    var listSuf = "_list";
    var pageSuf = "_list_pager";
    var cntSuf  = "_list_count";
    var manualInfo = {};
    ...
    for(var i = 0; i < len; i++) {
      Util.$propcopy(Service.SearchResult.MANUAL_INFO_TEMP, manualInfo);
      manualInfo.id = targets[i];
      manualInfo.manualName = Service.SearchResult.PUBTYPE_TO_NAME[targets[i]];
      
      // リストの削除
      this.doRemoveList(manualInfo);
      
      // 非表示化
      Element.$addClassName($(manualInfo.manualName + rootSuf), "invisible");
      Element.$addClassName($(manualInfo.manualName + listSuf), "invisible");
      Element.$addClassName($(manualInfo.manualName + pageSuf), "invisible");
      Element.$addClassName($(manualInfo.manualName + cntSuf), "invisible");
      
      // 展開/縮小アイコンの初期化
      expandElm = Util.Selector.$select("div.table_expander",
        $(manualInfo.manualName + rootSuf))[0];
      Element.$addClassName(expandElm, "expander_icon_close");
      Element.$removeClassName(expandElm, "expander_icon_expanded");
      
      // ページャの初期化
      this.doChangePager(manualInfo, false);
      
      // ラベルの初期化
      this.showResultStatus(manualInfo, false);
    }
