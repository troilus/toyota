...
Contents.$processForRMAndNCF = function() {
  ...
  try {
    ...
    // コンテンツ内表示モード
      ...
      category = $$("span.globalCategory")[0] ? 
          $$("span.globalCategory")[0].innerHTML : "";
      
      // カテゴリ"C", "Y", "J"の何れかの場合は診断フロー用INITの呼び出し
      if(category && Util.$getIndexOfArray(diagCategory, category) != -1) {
        // 診断フロー初期化呼び出し
...
Contents.$doClickReferenceLnk = function(linkKey) {
  ...
  try {
    ...
    if(!Contents.showMode) {
      ...
      Contents.myParent.$callbackReference(linkKey);
